...
const tabs = {
    ...
    'forecast-tab': document.getElementById('forecast-tab'),
    'leads-tab': document.getElementById('leads-tab'),
    'call-logs-tab': document.getElementById('call-logs-tab')
};
...
function setupEventListeners() {
    ...
    if (cardLeads) cardLeads.addEventListener('click', () => switchTab('leads-tab'));
    
    const cardCustomers = document.getElementById('card-total-customers');
    if (cardCustomers) cardCustomers.addEventListener('click', () => switchTab('call-logs-tab'));
    
    const cardLastTrained = document.getElementById('card-last-trained');
    if (cardLastTrained) cardLastTrained.addEventListener('click', () => switchTab('forecast-tab'));
...
function switchTab(targetId) {
    ...
    if (tabs[targetId]) {
        ...
        // Change page title based on tab
        const pageTitle = document.getElementById('page-title');
        if (targetId === 'dashboard-tab') pageTitle.textContent = 'Overview Dashboard';
        else if (targetId === 'forecast-tab') pageTitle.textContent = 'Forecasted Trends';
        else if (targetId === 'leads-tab') pageTitle.textContent = 'Analysed Policies';
        else if (targetId === 'call-logs-tab') pageTitle.textContent = 'Recent Call Logs';
    };
}
...
async function loadDashboard() {
    try {
        ...
        document.getElementById('stat-total-calls').textContent = data.stats.total_calls.toLocaleString();
        ...
        if (data.stats.latest_lead_precision) {
            document.getElementById('info-lead-prec').textContent = (data.stats.latest_lead_precision * 100).toFixed(1) + '%';
        }
        if (data.stats.latest_lead_recall) {
            document.getElementById('info-lead-rec').textContent = (data.stats.latest_lead_recall * 100).toFixed(1) + '%';
        }
        if (data.stats.latest_lead_f1_score) {
            document.getElementById('info-lead-f1').textContent = (data.stats.latest_lead_f1_score * 100).toFixed(1) + '%';
        }
    } catch (e) {
        console.error(e);
    }
...
async function loadLeads() {
    try {
        const response = await fetchWithAuth('/leads/predicted/all?limit=1000');
...
async function loadForecast() {
    try {
        const data = await fetchWithAuth('/predictions/forecast?days=30');
        ...
        const values = data.forecast.map(item => item.volume);

        // Styling configuration matching Insure AI theme
        ...
        forecastChartInstance = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: values,
                    borderColor: '#38bdf8', // primary blue
            ...
            options: {
                ...
                scales: {
                    ...
                    y: {
                        grid: { color: 'rgba(255, 255, 255, 0.05)' },
                        ticks: { color: 'rgba(255, 255, 255, 0.7)', beginAtZero: true, precision: 0 }
                    }
                }
            }
        });
    } catch (e) {
        console.warn("Forecast data not available:", e.message);
    }
}

async function loadCallLogs() {
    try {
        const data = await fetchWithAuth('/call-logs?limit=20');
        ...
        data.forEach(log => {
            let badgeClass = 'warning';
            if (log.outcome === 'conversion') badgeClass = 'success';
            else if (log.outcome === 'rejected') badgeClass = 'danger';
            ...
            htmlStr += `
                <tr>
                    ...
                    <td>${dateStr}</td>
                    <td>${log.call_duration.toFixed(1)}</td>
...
// Modal Logic
document.addEventListener('DOMContentLoaded', () => {
    // Removed Leads and Customers modals
...
async function handleFileUpload(file, endpoint, type) {
    ...
    try {
        ...
        showToast(`Successfully uploaded ${result.record_count} ${type}!`, 'success', 5000);
