...
function setupEventListeners() {
    ...
    if (cardCustomers) cardCustomers.addEventListener('click', () => switchTab('customers-tab'));
    
    const cardLastTrained = document.getElementById('card-last-trained');
    if (cardLastTrained) cardLastTrained.addEventListener('click', () => switchTab('models-tab'));
        ...
        data.forEach(lead => {
            ...
            htmlStr += `
                <tr>
                    ...
                    <td><i class="ph-fill ph-lightbulb" style="color:var(--warning); margin-right:8px;"></i>${lead.dialogue_prompt || lead.top_reasons[0]}</td>
        ...
        data.forEach(cust => {
            ...
            htmlStr += `
                <tr>
                    ...
                    <td><i class="ph-fill ph-warning-circle" style="color:var(--danger); margin-right:8px;"></i>${cust.dialogue_prompt || cust.top_reasons[0]}</td>
        ...
        items.forEach(lead => {
            ...
            htmlStr += `
                <tr>
                    ...
                    <td><span class="badge ${badgeClass}">${scorePercent}% Score</span></td>
                    <td><i class="ph-fill ph-lightbulb" style="color:var(--warning); margin-right:8px;"></i>${lead.dialogue_prompt || lead.top_reasons[0] || 'Unknown'}</td>
                    <td>${lead.contact_number}</td>
                </tr>
            `;
        });
        ...
        items.forEach(cust => {
            ...
            htmlStr += `
                <tr>
                    ...
                    <td><span class="badge ${badgeClass}">${scorePercent}% Risk</span></td>
                    <td><i class="ph-fill ph-warning-circle" style="color:var(--danger); margin-right:8px;"></i>${cust.dialogue_prompt || cust.top_reasons[0] || 'Unknown'}</td>
                    <td><span class="badge ${cust.sentiment === 'Negative' ? 'danger' : (cust.sentiment === 'Positive' ? 'success' : 'warning')}">${cust.sentiment}</span></td>
                    <td>${cust.contact_number || 'N/A'}</td>
                </tr>
            `;
