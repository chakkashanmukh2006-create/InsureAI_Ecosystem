...
class CustomerPredictor:
    ...
    def predict_all(self, db: Session) -> list[dict]:
        ...
        while True:
            ...
            scores: np.ndarray = model.decision_function(X_scaled)
            preds: np.ndarray = model.predict(X_scaled)
    ...
    def get_high_risk(self, db: Session) -> list[dict]:
        ...
        predictions = (
            ...
            .filter(CustomerPrediction.is_fraud == True)
            .order_by(desc(CustomerPrediction.anomaly_score))
            .limit(100)
            .all()
        )
    ...
    def _categorise(self, churn_prob: float) -> str:
        """Categorisation removed for anomaly detection."""
        return "Unknown"
