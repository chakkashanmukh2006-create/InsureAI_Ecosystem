...
function switchTab(targetId) {
    ...
    const titles = {
        ...
        'leads-tab': 'Suspicious Leads',
        'customers-tab': 'Potential Fraud Cases'
    };
...
async function loadLeads() {
    try {
        ...
        let htmlStr = '';
        data.forEach(lead => {
            const scorePercent = lead.anomaly_score.toFixed(2);
            let badgeClass = 'warning';
            if (lead.is_fraud) badgeClass = 'danger';
...
async function loadCustomers() {
    try {
        ...
        let htmlStr = '';
        data.forEach(cust => {
            const scorePercent = cust.anomaly_score.toFixed(2);
            let badgeClass = 'warning';
            if (cust.is_fraud) badgeClass = 'danger';
