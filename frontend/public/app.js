...
async function loadRetailForecast() {
    if (serviceStatus.predictive === 'offline') {
        showToast('Predictive Intelligence (Port 8002) is offline. Cannot generate retail forecast.', 'error');
        return;
    }
    ...
    let endpoint = `/retail/forecast?level=${level}`;
    if (level !== 'store' && name) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const historyData = data.history.map(val => val !== null ? val : NaN);
            ...
            retailChartInstance = new Chart(ctx, {
                ...
                data: {
                    labels: data.dates,
                    datasets: [
                        { label: 'Historical Sales', data: historyData, borderColor: '#ffffff', backgroundColor: 'transparent', borderWidth: 2, tension: 0.1, pointRadius: 0 },
                        { label: 'Prophet', data: data.predictions.Prophet, borderColor: '#2196F3', backgroundColor: 'transparent', borderWidth: 2, borderDash: [5, 5], tension: 0.1, pointRadius: 0 },
                        { label: 'XGBoost', data: data.predictions.XGBoost, borderColor: '#4CAF50', backgroundColor: 'transparent', borderWidth: 2, borderDash: [5, 5], tension: 0.1, pointRadius: 0 },
                        { label: 'SARIMA', data: data.predictions.SARIMA, borderColor: '#FF9800', backgroundColor: 'transparent', borderWidth: 2, borderDash: [5, 5], tension: 0.1, pointRadius: 0 },
                        { label: 'Holt-Winters', data: data.predictions.HoltWinters, borderColor: '#9C27B0', backgroundColor: 'transparent', borderWidth: 2, borderDash: [5, 5], tension: 0.1, pointRadius: 0 },
                        { label: 'SMA Baseline', data: data.predictions.SMA, borderColor: '#F44336', backgroundColor: 'transparent', borderWidth: 2, borderDash: [5, 5], tension: 0.1, pointRadius: 0 }
                    ]
                },
                options: {
                    responsive: true,
                    ...
                    scales: {
                        x: { 
                            ...
                            title: { display: true, text: 'Timeline (Days)', color: 'rgba(255,255,255,0.7)' }
                        },
                        y: { 
                            grid: { color: 'rgba(255,255,255,0.05)' }, 
                            ticks: { color: 'rgba(255,255,255,0.5)' },
                            title: { display: true, text: 'Weekly Sales Volume', color: 'rgba(255,255,255,0.7)' }
                        }
                    },
                    interaction: { mode: 'index', intersect: false }
                }
        ...
        console.error("Failed to load retail forecast:", e);
        showToast('Failed to train models. See console.', 'error');
        const overlay = document.getElementById('retail-loading-overlay');
        if (overlay) overlay.style.display = 'none';
    }
}
